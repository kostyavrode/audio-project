...
public class RabbitMQConsumer : BackgroundService, IRabbitMQConsumer
{
    ...
    private const string QueueName = "audio-service.groups-events";
    ...
    private async Task ProcessMessageAsync(string routingKey, string message, CancellationToken cancellationToken)
    {
        ...
        switch (routingKey)
        {
            case "UserJoinedGroupEvent":
                await HandleUserJoinedGroupEventAsync(eventData, dbContext, eventId, cancellationToken);
                break;

            case "UserLeftGroupEvent":
                await HandleUserLeftGroupEventAsync(eventData, dbContext, eventId, cancellationToken);
                break;
        ...
        CancellationToken cancellationToken)
    {
        ...
        dbContext.GroupMembers.RemoveRange(groupMembers);

        var audioChannels = await dbContext.AudioChannels
            .Where(c => c.GroupId == groupId)
            .ToListAsync(cancellationToken);

        dbContext.AudioChannels.RemoveRange(audioChannels);

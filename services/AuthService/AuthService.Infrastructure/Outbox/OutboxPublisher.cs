...
public class OutboxPublisher : BackgroundService
{
    ...
    {
        // Создаем scope для получения scoped сервисов (DbContext, RabbitMQPublisher)
        using var scope = _serviceProvider.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<OutboxDbContext>();
        var rabbitMqPublisher = scope.ServiceProvider.GetRequiredService<IRabbitMQPublisher>();
        
        // Получаем непубликованные сообщения (пакетами для производительности)
        ...
        {
            return; // Нет сообщений для публикации
        }
